...
def leave_one_out(data, col_names, protein):
    """Leave-one-out cross validation with random sampling."""
    assert len(data) == 600
    iupac, mscore = list(zip(*data))
    protein_idx = col_names.index(protein)
    binding = []
    ...
    for i in range(600):
        if mscore[i][protein_idx] >= 2:
            ...
            nonbinding.append(iupac[i])
    features = binding + nonbinding
    label = [1] * len(binding) + [0] * len(nonbinding)

    binding_train = []
    test_feature = []
    nonbinding_train = []
    test_label = []
    for test_index in range(600):
        x_test = features[test_index]
        y_test = label[test_index]
        test_feature.append(x_test)
        test_label.append(y_test)
        if y_test == 1:
            binding_train.append(features[0:len(binding) - 1])
            nonbinding_train.append(features[len(binding) - 1:])
        else:
            binding_train.append(features[0:len(binding)])
            nonbinding_train.append(features[len(binding):])

    return binding_train, test_feature, nonbinding_train, test_label
...
def train_and_test(use_edge=False, n_folds=10, n_states=5, max_iter_1=50, max_iter_2=None, delta=1e-5,
    ...
    binding_train, test_X, binding_test, test_y = leave_one_out(data, col_names, target_protein)

    for fold_iter, (bind_train, test_iupac, nonbind_train, test_label) in enumerate(zip(binding_train, test_X,
                                                                                        binding_test, test_y)):
        logging.info('*' * 50)
        ...
        binding_hmm_trained = initHMM.initHMM(states, emissions, state_transition_probabilities=bind_param['hmm'][
            'state_transition_probabilities'],
                                              emission_probabilities=bind_param['hmm']['emission_probabilities'])
        nonbinding_hmm_trained = initHMM.initHMM(states, emissions, state_transition_probabilities=nonbind_param['hmm'][
            'state_transition_probabilities'],
        ...
        # Testing
        cv_iupac.append(test_iupac)
        iupac = re.split(r"\([^\)]*$", test_iupac, 1)[0]
        ...
        # use posterior
        if logsumexp(bind_fwd_probs.iloc[:, -1]) + log_by_chance_bind_prob >= logsumexp(
                nonbind_fwd_probs.iloc[:, -1]) + log_by_chance_nobind_prob:
            testpred_posterior = 1
        else:
            testpred_posterior = 0

        cv_label.append(test_label)
        cv_pred.append(testpred)
        cv_pred_posterior.append(testpred_posterior)
